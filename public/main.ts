...
// ---------------------------- Tipagens básicas -----------------------------
// Mantém os modelos de dados em um só lugar para simplificar leituras.
// Caso adicione novos campos ao app, atualize as interfaces aqui primeiro.

// Tipos e modelos colocados em um único arquivo para deixar tudo mais direto
type CategoriaProduto = 'Pizza' | 'Refrigerante' | 'Sobremesa' | 'Outro';
...
const sections = Array.from(document.querySelectorAll<HTMLElement>('main .view'));
const feedbackBox = document.getElementById('feedback') as HTMLDivElement;
let feedbackTimeout: number | undefined;

const clienteForm = document.getElementById('cliente-form') as HTMLFormElement;
const clienteNome = document.getElementById('cliente-nome') as HTMLInputElement;
const clienteTelefone = document.getElementById('cliente-telefone') as HTMLInputElement;
const clienteEndereco = document.getElementById('cliente-endereco') as HTMLInputElement;
const clienteCancel = document.getElementById('cliente-cancel') as HTMLButtonElement;
const clienteTitle = document.querySelector('[data-cliente-form-title]') as HTMLElement;
const clientesTableBody = document.querySelector('#clientes-table tbody') as HTMLTableSectionElement;

const produtoForm = document.getElementById('produto-form') as HTMLFormElement;
const produtoNome = document.getElementById('produto-nome') as HTMLInputElement;
const produtoCategoria = document.getElementById('produto-categoria') as HTMLSelectElement;
const produtoPreco = document.getElementById('produto-preco') as HTMLInputElement;
const produtoAtivo = document.getElementById('produto-ativo') as HTMLInputElement;
const produtoCancel = document.getElementById('produto-cancel') as HTMLButtonElement;
const produtoTitle = document.querySelector('[data-produto-form-title]') as HTMLElement;
const produtosTableBody = document.querySelector('#produtos-table tbody') as HTMLTableSectionElement;

const pedidoForm = document.getElementById('pedido-form') as HTMLFormElement;
const pedidoCliente = document.getElementById('pedido-cliente') as HTMLSelectElement;
const pedidoProduto = document.getElementById('pedido-produto') as HTMLSelectElement;
const pedidoQuantidade = document.getElementById('pedido-quantidade') as HTMLInputElement;
const pedidoAddItem = document.getElementById('pedido-add-item') as HTMLButtonElement;
const pedidoForma = document.getElementById('pedido-forma') as HTMLSelectElement;
const pedidoReset = document.getElementById('pedido-reset') as HTMLButtonElement;
const pedidoItensBody = document.querySelector('#pedido-itens tbody') as HTMLTableSectionElement;
const pedidosTableBody = document.querySelector('#pedidos-table tbody') as HTMLTableSectionElement;
const totalBrutoSpan = document.querySelector('[data-total-bruto]') as HTMLElement;
const totalDescontoSpan = document.querySelector('[data-total-desconto]') as HTMLElement;
const totalLiquidoSpan = document.querySelector('[data-total-liquido]') as HTMLElement;
const totalObservacao = document.querySelector('[data-total-observacao]') as HTMLElement;

const relatorioDiaBody = document.querySelector('#relatorio-dia tbody') as HTMLTableSectionElement;
const relatorioMesBody = document.querySelector('#relatorio-mes tbody') as HTMLTableSectionElement;
const historicoSelect = document.getElementById('historico-cliente-select') as HTMLSelectElement;
const historicoLista = document.getElementById('historico-lista') as HTMLDivElement;

const resetDadosBtn = document.getElementById('reset-dados') as HTMLButtonElement;
const limparDadosBtn = document.getElementById('limpar-dados') as HTMLButtonElement;
...
let state: AppState = carregarEstado();
...
function gerarEstadoPadrao(): AppState {
  return {
    clientes: [
...
function gerarEstadoVazio(): AppState {
  return {
    clientes: [],
    produtos: [],
    pedidos: [],
...
function carregarEstado(): AppState {
  ...
  try {
    const bruto = JSON.parse(salvo) as AppState;
    return {
...
function trocarSecao(id: string): void {
...
function mostrarFeedback(texto: string, tipo: 'success' | 'error' = 'success'): void {
...
function salvarCliente(evento: SubmitEvent): void {
...
function cancelarEdicaoCliente(): void {
  clienteForm.reset();
  clienteForm.dataset.editId = '';
  clienteCancel.classList.add('hidden');
  clienteTitle.textContent = 'Novo cliente';
...
function tratarAcoesClientes(evento: MouseEvent): void {
  const alvo = evento.target as HTMLElement;
  if (!alvo.dataset.action) return;
...
function salvarProduto(evento: SubmitEvent): void {
  evento.preventDefault();
  const nome = produtoNome.value.trim();
  const categoria = (produtoCategoria.value as CategoriaProduto) || 'Outro';
...
function tratarAcoesProdutos(evento: MouseEvent): void {
  const alvo = evento.target as HTMLElement;
  if (!alvo.dataset.action) return;
  const id = Number(alvo.dataset.id);
  if (alvo.dataset.action === 'editar') {
...
function adicionarItemAoPedido(): void {
...
function tratarAcoesItensPedido(evento: MouseEvent): void {
  const alvo = evento.target as HTMLElement;
  if (!alvo.dataset.index) return;
...
function finalizarPedido(evento: SubmitEvent): void {
  evento.preventDefault();
  ...
  const forma = (pedidoForma.value as FormaPagamento) || 'Dinheiro';
  ...
  if (cliente) cliente.historicoPedidosIds.push(novoPedido.id);
...
function tratarAcoesPedidos(evento: MouseEvent): void {
  const alvo = evento.target as HTMLElement;
  if (!alvo.dataset.action) return;
  const id = Number(alvo.dataset.id);
  if (alvo.dataset.action === 'remover') {
...
function renderPedidoSelects(): void {
...
function renderPedidoItens(): void {
  if (pedidoItens.length === 0) {
...
function renderTotaisPedido(): void {
  const forma = (pedidoForma.value as FormaPagamento) || 'Dinheiro';
  const resumo = calcularResumoPedido(pedidoItens, forma);
...
function renderPedidosTabela(): void {
...
function renderRelatorios(): void {
...
function renderHistoricoSelect(): void {
...
function renderHistoricoLista(): void {
...
function calcularResumoPedido(itens: PedidoItem[], forma: FormaPagamento): PedidoResumo {
  let totalBruto = 0;
  let totalPizzas = 0;
  let valorPizzas = 0;
  itens.forEach(item => {
...
function gerarCodigoFiscal(): string {
  const numero = Math.floor(Math.random() * 9000) + 1000;
  return `NF-${numero}`;
}

type JsPDFConstructor = new (options?: unknown) => JsPDFInstance;
interface JsPDFInstance {
  ...
  text(conteudo: string, x: number, y: number, options?: unknown): void;
  setLineWidth?(width: number): void;
  line?(x1: number, y1: number, x2: number, y2: number): void;
  rect?(x: number, y: number, w: number, h: number, style?: 'F' | 'S' | 'DF'): void;
  roundedRect?(x: number, y: number, w: number, h: number, rx: number, ry: number, style?: 'F' | 'S' | 'DF'): void;
  save(nome: string): void;
}

// Monta um PDF estilizado com os dados do pedido selecionado
function emitirComprovanteFiscal(pedidoId: number): void {
  ...
  const cliente = state.clientes.find(c => c.id === pedido.clienteId);
  const construtor = (window as unknown as { jspdf?: { jsPDF?: JsPDFConstructor } }).jspdf?.jsPDF;
  if (!construtor) {
